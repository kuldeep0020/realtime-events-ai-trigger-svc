#!/usr/bin/env python3
"""Real-estate persona demo event publisher.

Sends the realtor_session_abandoned trigger flow to Pulsar (or HTTP).
Default: 1 user, fixed anonymousId, matches existing Go demofire output.

Usage:
    uv run demo_realestate.py [OPTIONS]
    uv run demo_realestate.py --dry-run | jq .
...
DEFAULT_WRITE_KEY = "3DNyjJW7sRSqftUb1UQuMJdxlFw"
DEFAULT_ANON_ID = "anon_demo-re-001"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument("--cohort-size", type=int, default=1, help="Number of concurrent users")
    p.add_argument("--anonymous-id", default=DEFAULT_ANON_ID,
                   help="Fixed anonymousId for cohort-size=1 (ignored otherwise)")
    p.add_argument("--write-key", default=os.environ.get("WRITE_KEY_REALESTATE", DEFAULT_WRITE_KEY))
    ...
    p.add_argument("--speed", type=float, default=1.0,
                   help="Time multiplier: 2.0=half pace, 0=no sleep (stress)")
...
# ---------------------------------------------------------------------------
# HTTP publish fallback (uses stdlib urllib — no extra deps)
# ---------------------------------------------------------------------------
...
def run_user_flow(
    ...
    """Execute one user's realestate script. Returns number of events published."""
    steps = realestate_script(cfg, rng)
    sent = 0
    ...
    for step in steps:
        ...
        if args.target == "pulsar":
            ...
            if args.verbose:
                log.info(
                    "[RE] published: anon=%-22s type=%-8s event=%-28s ts=%s msgId=%s",
                    event["anonymousId"],
                    event["type"],
            ...
            if args.verbose:
                log.info("[RE] http sent: anon=%s type=%s event=%s",
...
def main(argv: list[str] | None = None) -> None:
    ...
    if args.cohort_size == 1:
        configs = [make_re_user_config(args.anonymous_id, rng=master_rng)]
    else:
        configs = [
            make_re_user_config(
                f"anon_demo_re_{master_rng.randint(0, 0xFFFFFFFF):08x}",
    ...
    if args.dry_run:
        # Single-user dry run; don't open Pulsar
        cfg = configs[0]
    ...
    def _run_one(cfg: REUserConfig, user_seed: int) -> int:
        user_rng = random.Random(user_seed)
        fk = Faker()
        if seed is not None:
            fk.seed_instance(user_seed)
